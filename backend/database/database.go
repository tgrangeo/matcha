...
func InsertUser(db *sql.DB, user models.User) {
	...
	_, err = db.Exec(insertStmt, user.Fname, user.Lname, user.Email, user.Birthdate, crypted, user.Bio, pq.StringArray(user.Imageurl), age, user.Gender, pq.Array(user.Desiredgender), user.Fame, pq.Array(user.Tags),
		pq.Array(user.Pokeball), pq.Array(user.Type), pq.Array(user.Userliked), pq.Array(user.Likedfrom), pq.Array(user.Seenfrom), pq.Array(user.Blocked), pq.Array(user.Convlist), utils.LocToJson(user.Coord), utils.NotifsToJson(user.Notifs), user.Isactive)
	if err != nil {
		panic(err)
	}
...
func GetUsers(db *sql.DB) []models.User {
	...
	for rows.Next() {
		...
		usr := models.User{id, fname, lname, email, birthdate, pass, bio, imageurl, age, gender, fame, desiredgender, tags, Type, pokeball, userliked, likedfrom, seenfrom, blocked, convlist, utils.JsonToLoc(coord), utils.JsonToNotifs(notifs), isactive}
		tab = append(tab, usr)
		if err != nil {
			fmt.Println(err)
		}
