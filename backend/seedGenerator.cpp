...
class user {
	  public:
		...
		std::vector<int> meLikeUsers;
		std::vector<int> userLikesMe;
...
{
	user users[1000];
	std::string genre[3] = {"Homme", "Femme", "Non-binaire"};
	...
	for (int i = 0; i < 1000 ; i++)
	{
		...
		users[i].lastname = names[randomInt(0, 999)] ;
	...
	for (int i = 0; i < 1000 ; i++)
	{
		...
		int userTags[users[i].sizeTags];
		for (int j = 0; j < users[i].sizeTags; j++)
			userTags[j] = randomInt(0, 99);
		...
		for (int j = 0; j < users[i].sizeTags; j++)
		{
			if (j != users[i].sizeTags - 1)
				users[i].TagsJson +=  std::to_string(userTags[j]) + ",";
			else
				users[i].TagsJson += std::to_string(userTags[j]);
		}
		users[i].TagsJson += "]";
		...
		for (int j = 0; j < size; j++){
			...
			if (isPossible(users[i], users[findbyName(users, names[newer])]) && std::find(users[i].meLikeUsers.begin(), users[i].meLikeUsers.end(), newer) == users[i].meLikeUsers.end() )
			{

				users[i].meLikeUsers.push_back(newer);
				users[newer].userLikesMe.push_back(i);
			}
		}
	...
	for (int i = 0; i < 1000; i++)
	{
		...
		for (std::vector<int>::iterator itu = users[i].userLikesMe.begin(); size >= 0 && itu != users[i].userLikesMe.end(); itu++)
		{
			if(std::find(users[i].meLikeUsers.begin(), users[i].meLikeUsers.end(), *itu) == users[i].meLikeUsers.end())
			{
				...
				users[*itu].userLikesMe.push_back(i);
			}
			size--;
		}
		users[i].meLikeUsersJson = "[";
		for (std::vector<int>::iterator it = users[i].meLikeUsers.begin() ; it != users[i].meLikeUsers.end(); it++)
		{
			if (it != (users[i].meLikeUsers.end() - 1))
				users[i].meLikeUsersJson +=  std::to_string(*it)  + ",";
			else
				users[i].meLikeUsersJson +=  std::to_string(*it);
		}
		users[i].meLikeUsersJson += "]";
		users[i].userLikesMeJson = "[";
		for (std::vector<int>::iterator it = users[i].userLikesMe.begin() ; it != users[i].userLikesMe.end(); it++)
		{
			if (it != (users[i].userLikesMe.end() - 1))
				users[i].userLikesMeJson +=  std::to_string(*it)  + ",";
			else
				users[i].userLikesMeJson +=  std::to_string(*it);
		}
		users[i].userLikesMeJson += "]";
		for (std::vector<int>::iterator itj = users[i].meLikeUsers.begin() ; itj != users[i].meLikeUsers.end(); itj++)
		{
		...
		users[i].MatchsJson = "[";
		for (std::vector<int>::iterator it = users[i].Matchs.begin() ; it != users[i].Matchs.end(); it++)
		{
			if (it != (users[i].Matchs.end() - 1))
				users[i].MatchsJson +=  std::to_string(*it)  + ",";
			else
				users[i].MatchsJson += std::to_string(*it);
		}
		users[i].MatchsJson += "]";
		std::cout << "INSERT INTO MatchaBDD.Users ( lastname, firstname, email ,password, orientationId, bio, dms, age, locationX, locationY, tags, image, score, meLikeUsers, userLikesMe, matchs) VALUES (\"" << users[i].lastname << "\",\"" << users[i].firstname << "\",\"" << users[i].email << "\",\"" << users[i].password << "\"," << users[i].orientationId << ",\"" << users[i].bio << "\",\"" << "[]" << "\"," << users[i].age << "," << users[i].locationX << "," << users[i].locationY << ",\"" << users[i].TagsJson << "\",\"" << users[i].image << "\"," << users[i].score << ",\"" << users[i].meLikeUsersJson << "\",\"" << users[i].userLikesMeJson << "\",\"" << users[i].MatchsJson << "\");" << std::endl;
		}
}
